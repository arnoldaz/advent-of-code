...
for line in lines:
    ...
    bricks.append(Brick(Point3d(start_x, start_y, start_z), Point3d(end_x, end_y, end_z)))

# Lowest brick at the start
bricks.sort(key=lambda brick: min(brick.start.z, brick.end.z))

def brick_has_block(brick: Brick, block_position: Point3d) -> bool:
...
def brick_can_fall(brick: Brick, all_bricks: list[Brick]) -> bool:
    ...
    if brick.start.y == brick.end.y and brick.start.z == brick.end.z:
        ...
        for x in range(min_x, max_x + 1):
            block_position = Point3d(x, brick.start.y, brick.start.z - 1)
    ...
    # on Y axis
        ...
        for y in range(min_y, max_y + 1):
            block_position = Point3d(brick.start.x, y, brick.start.z - 1)
            for other_brick in all_bricks:
                if brick_has_block(other_brick, block_position):
                    return False
    # on Z axis
        ...
        block_position = Point3d(brick.start.x, brick.start.y, min_z - 1)
        for other_brick in all_bricks:
            if brick_has_block(other_brick, block_position):
                return False
    ...
    while anything_fall:
        ...
        for i, brick in enumerate(new_bricks):
            if brick_can_fall(brick, new_bricks):
                new_bricks[i] = Brick(Point3d(brick.start.x, brick.start.y, brick.start.z - 1), Point3d(brick.end.x, brick.end.y, brick.end.z - 1))
                fall_blocks[i] = True
                anything_fall = True
...
print(f"{count_disintegratable_bricks(all_new_bricks)=}") # runs for ~10min
print(f"{count_not_disintegratable_brick_sum(all_new_bricks)=}") # runs for ~1h
